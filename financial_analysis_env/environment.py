...
class FinancialAnalysisEnvironment:
    ...
    def reset(self) -> FinancialAnalysisObservation:
    ...
    def step(self, action: FinancialAnalysisAction) -> FinancialAnalysisObservation:
        ...
        if _current_task is None:
            _current_task = random.choice(TASKS)  # auto-fix for testing
    ...
    def _calculate_reward(self, action: FinancialAnalysisAction) -> float:
        ...
        # ── HARD (UPGRADED) ──────────────────────
            ...
            # Now apply check AFTER calculation
            if risks_found == 0:
                return 0.0  # minimal effort reward
            ...
            # 4. Structure enforcement (scaled)
            issue_count = len(action.identified_issues)
            structure_score = min(issue_count / 3, 1)
            reward *= structure_score

            # 5. Reasoning bonus (differentiator)
            reasoning_hits = sum(1 for word in ["because", "due to", "driven by", "as a result"] if word in analysis)
            reward += 0.05 * min(reasoning_hits, 2)
